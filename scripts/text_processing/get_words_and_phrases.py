...
def build_word2phrase(words_df, unique_phrase_df, output_path) -> pd.DataFrame:
    ...
    for index, word in words_df["word"].items():
        phrase_mask = (
            unique_phrase_df["phrase"].str.split().apply(lambda words: word in words)
        )
        phrase_indices = unique_phrase_df.index[phrase_mask].tolist()
        rows.extend(
            {"word_idx": index, "phrase_idx": phrase_idx}
...
def main():
    ...
    phrase_merges_df = pd.read_csv(
        phrase_merges_path, index_col="index", keep_default_na=False
    )
