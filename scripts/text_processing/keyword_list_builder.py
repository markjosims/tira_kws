...
def main():
    ...
    word_df = pd.read_csv(WORDS_CSV, keep_default_na=False)
    print(f"Loaded {len(word_df)} unique words from {WORDS_CSV}")

    unique_phrase_df = pd.read_csv(PHRASES_CSV, keep_default_na=False)
    print(f"Loaded {len(unique_phrase_df)} unique phrases from {PHRASES_CSV}")

    word2phrase = pd.read_csv(WORD2PHRASE_PATH, keep_default_na=False)
    print(f"Loaded word2phrase mapping from {WORD2PHRASE_PATH}")

    phrase2records = pd.read_csv(PHRASE2RECORDS_PATH, keep_default_na=False)
