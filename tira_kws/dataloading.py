...
def load_supervisions_df() -> pd.DataFrame:
    ...
    supervisions_df["fst_text"] = supervisions_df["custom"].apply(
        lambda x: x["fst_text"]
    )
    ...
    supervisions_df["translation"] = supervisions_df["custom"].apply(
        lambda x: x.get("translation", "")
    )
...
def load_elicitation_cuts(index_list: Optional[List[int]] = None) -> CutSet:
    ...
    if index_list is not None:
        ...
        cuts = cuts.to_eager()
        assert len(cuts) == len(index_list), (
            f"Expected {len(index_list)} cuts after filtering but got {len(cuts)}"
        )
